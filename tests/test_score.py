...
def test_verify_anomalies_selects_only_strong_disagreement(con, monkeypatch):
    ...
    def fake_classify(client, review_id, place_id, text, term, model):
        ...
        return {"review_id": review_id, "place_id": place_id, "relevant": True, "sentiment": -1.0, "reason": "second avis", "failed": False}
...
def test_verify_anomalies_never_arbitrates_toward_rating(con, monkeypatch):
    ...
    def fake_classify(client, review_id, place_id, text, term, model):
        ...
        return {"review_id": review_id, "place_id": place_id, "relevant": True, "sentiment": -0.9, "reason": "confirmé", "failed": False}
...
def test_verify_anomalies_is_idempotent(con, monkeypatch):
    ...
    def fake_classify(client, review_id, place_id, text, term, model):
        calls.append(review_id)
        return {"review_id": review_id, "place_id": place_id, "relevant": True, "sentiment": -0.9, "reason": "x", "failed": False}
